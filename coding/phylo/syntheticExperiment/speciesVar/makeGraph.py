...
def saveFig(statsFileName, title, outFileName):
	...
	while 1:
		...
		for j in range(4,9):
			...
			#curData = [x/max(curData) for x in curData]
	...
	plt.ylabel("Normalized Pairwise Pathlength Distance", **fontMed)
...
def main():
	algs = ["CS", "CW", "MSC"]
	fileNames = algs
	titles = ["PPLD vs. Number of Species for " + alg for alg in algs]
	outfiles = [alg + "acc.eps" for alg in algs]
